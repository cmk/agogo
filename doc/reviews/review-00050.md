...
<!-- gh-id: 3167843289 -->
### Copilot on [`crates/stdio/src/snapshot.rs:690`](https://github.com/cmk/agogo/pull/50#discussion_r3167843289) (2026-04-30 12:19 UTC)
...
This PR adds the agogo-owned snapshot observation payload + RT-safe handoff implementation in `agogo-stdio`, and documents the v1 wire contract/schema for downstream (TUI/stdio-core) consumers.

**Changes:**
- Add `agogo_stdio::snapshot` with `AgogoSnapshot` v1 (serde + fixed-point JSON number wrappers), an atomic RT→async `SnapshotSlot`, and a stdio-core-shaped observation publisher.
- Add/align documentation for the v1 schema/RT boundary and update sprint/version tracking docs.
- Export the new snapshot module from `agogo-stdio` and add the missing direct `serde` dependency.
...
| crates/stdio/src/snapshot.rs | Implements snapshot types, fixed-point serde wrappers, atomic slot, publisher, and tests. |
| crates/stdio/src/lib.rs | Exposes the new `snapshot` module and re-exports `AgogoSnapshot`/`SnapshotSlot`. |
| crates/stdio/Cargo.toml | Adds `serde` to dependencies (required for derives). |
...
<!-- gh-id: 3167843344 -->
### Copilot on [`crates/stdio/src/snapshot.rs`](https://github.com/cmk/agogo/pull/50#discussion_r3167843344) (2026-04-30 12:19 UTC)
...
<!-- gh-id: 3167843358 -->
### Copilot on [`crates/stdio/src/snapshot.rs:777`](https://github.com/cmk/agogo/pull/50#discussion_r3167843358) (2026-04-30 12:19 UTC)
