...
1. **Inventing a placeholder filename.** The local review transition
   ...
   (the script queries the repo's highest issue/PR number via `gh api`
   and adds one). Without a visible `review-NNNNN.md` in the directory,
   an agent that doesn't find the script might invent an ad-hoc name
   like `review-draft.md` or `review-pending.md` instead of running it.
