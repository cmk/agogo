...
- **Per-channel `OutputFormat` enum as the dispatch type.** Shape
  ...
  carries its own routing specifier. Today `Channel` is already split
  by routing target (`Midi`, `Din`, `Cv`) with target-specific role
  enums in `crates/core/src/channel/role.rs`; v0.4 should extend that
  model rather than introducing a separate flat routing enum.
  Name the final routing/role boundary in that sprint.
...
- **MIDI Clock at 24 PPQN, derived from the master tick stream.**
  At agogo's 960 PPQN master, one MIDI tick (0xF8) every
  `PPQN/24` master ticks. Send
