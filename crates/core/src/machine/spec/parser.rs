...
/// forms.
///
/// This is the inverse of `Display for ChannelSpec`'s `delay={ms_int}.
/// {frac:03}` form. f64 cannot represent decimal fractions like `0.116`
/// exactly — `0.116 × 10⁻³` round-trips through `F064FD06.ceil` to
/// `Micro(117)`, one µs above the source value. Splitting on `.` and
/// parsing both halves as integers sidesteps the float entirely.
fn parse_decimal_ms(s: &str) -> Option<Micro> {
    ...
    // signed). f64 path handles `1e3`, `1_000`, etc.
    ...
    // user-friendly direction) for longer inputs.
