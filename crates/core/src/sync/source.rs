...
impl PhaseSource {
    /// Phase in cycles \[0, 1) at the given absolute sample index.
    /// See the variant docs for the unit each variant uses.
    pub fn phase_at_sample(&mut self, n: u64) -> f32 {
        match self {
            ...
            PhaseSource::External { pll, .. } => pll.state().phase as f32,
        }
    }

    /// Feed a block of audio samples into the clock. No-op for
    /// `Internal`; routes to detector → PLL for `External`. If a block
    /// contains no detected peaks, the PLL is free-run for one step so
    /// callers can use this as a steady-tick driver.
...
mod tests {
    ...
    fn external_feed_samples_advances_pll() {
        ...
        // After feeding, External's phase is whatever the PLL last
        // settled to (≈ 0 since phase resets at each pulse boundary).
