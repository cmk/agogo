...
#
# Default mode scans only lines the commit ADDS (drops unchanged context,
# deletions, and pre-existing content). `--tree` scans every tracked file
# for public-release checks. Both modes check for:
...
case "${1:-}" in
  ...
  "-h" | "--help")
    cat <<'USAGE'
usage: scripts/check_pii.sh [--staged|--tree]
...
USAGE
  ...
  *)
    ...
    echo "usage: scripts/check_pii.sh [--staged|--tree]" >&2
    exit 2
    ;;
esac
