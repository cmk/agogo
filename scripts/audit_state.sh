#!/usr/bin/env bash
#
# audit_state.sh — early-exit helper for recurring audit agents
#
...
#
# Usage:
#   audit_state.sh since-last <name> [path1 path2 ...]
...
#
# Example agent loop:
#   files=$(scripts/audit_state.sh since-last proptest crates/ tests/)
...
git_dir=$(git rev-parse --git-dir 2>/dev/null) || {
    echo "audit_state.sh: not inside a git work tree" >&2
    exit 2
}
...
case "$cmd" in
    ...
    mark)
        ...
        echo "audit_state: pinned $name -> $head_sha" >&2
        ;;
    ...
    since-last)
        ...
        if [ -f "$pin_file" ]; then
            ...
            if git cat-file -e "$last_sha" 2>/dev/null; then
                ...
                echo "audit_state: pinned SHA $last_sha unreachable; auditing all" >&2
                git ls-files -- "$@"
            fi
        else
